...
window.Shell = (() => {
  ...
  function registerAccount({ username, password, name, avatarColor, avatarEmoji, seedSnapshot }) {
    ...
    accts[uname] = { password, snapshot: snapshotCurrentState(), syncCodeClaimed: !!seedSnapshot };
    setAccounts(accts);
  ...
  // Anti-macro: real thumbs can't sustain much more than ~8-9 taps/sec for long. Anything
  ...
  const CLICK_RATE_CAP = 8; // max counted clicks per rolling second
  let clickTimestamps = [];
  ...
  const PASSIVE_CAP_ABS = 200000;
  // ---------- autoclick detection ----------
  const AUTOCLICK_KEY = "nj_autoclick_state"; // { day, offenseCount, lockUntil }
  const AUTOCLICK_WARN_WINDOW_MS = 20000; // sustained max-rate clicking for this long triggers a check
  ...
  function devLiveUpdateHTML() {
    ...
    return `
      ...
      <div class="dev-revert-note">Publishing locks out everyone whose local CURRENT_VERSION doesn't match "New version number" behind a full-screen download prompt — including this browser, unless this build's CURRENT_VERSION already matches what you publish. Bump CURRENT_VERSION in chat.js to that same value before sending out the updated files, so the new files stop showing the lockout for themselves.</div>`;
  }
  ...
  function openProfileModal() {
    ...
    if (syncVeil && syncWrap) syncVeil.onclick = () => syncWrap.classList.add("revealed");
    const copySyncBtn = wrap.querySelector("[data-copy-sync]");
    if (copySyncBtn) copySyncBtn.onclick = () => {
    ...
    const toggleImportBtn = wrap.querySelector("[data-toggle-import]");
    const importBox = wrap.querySelector("[data-import-box]");
    if (toggleImportBtn && importBox) toggleImportBtn.onclick = () => {
      importBox.style.display = importBox.style.display === "none" ? "block" : "none";
    };
    const importSubmit = wrap.querySelector("[data-import-submit]");
    if (importSubmit) importSubmit.onclick = () => {
      const code = wrap.querySelector("[data-import-input]").value;
      if (!code.trim()) return;
      const result = importAccountCode(code);
      if (result === true) {
        notify("Account loaded. Reloading…");
        setTimeout(() => window.location.reload(), 700);
      } else if (result === "stale") {
        notify("That code is out of date — this account has newer progress than what's saved in it, so it can't be restored.");
      } else {
        notify("That code couldn't be read. Double-check it and try again.");
      }
    };
  ...
  function openSettingsModal(startScreen) {
    ...
    function render() {
      wrap.innerHTML = screen === "personalization" ? personalizationHTML() : screen === "password" ? passwordHTML() : mainHTML();
      wrap.querySelector("[data-close-settings]").onclick = closeAllOverlays;
      if (screen === "main") bindMain(); else if (screen === "password") bindPassword(); else bindPersonalization();
    }
  ...
  function renderLoginGate({ switching = false } = {}) {
    ...
    function bindLogin() {
      ...
      wrap.querySelector("[data-lg-login]").onclick = () => {
        ...
        if (loginToAccount(username, password)) {
  ...
  function statsFilteredBetLog(game) {
    ...
    const log = getBetLog().filter((b) => known.has(b.game));
